...
class IdentityServiceApplicationTests extends ContainerBase {
  ...
  void givenInvalidUser_whenCreatingUser_thenReturnBadRequest() {
    ResponseEntity<ProblemDetail> problemDetailResponseEntity = restTemplate.postForEntity(
        usersV1BaseUrl, new UserRequestDto(), ProblemDetail.class);
    ...
    assertThat(body.getErrors().size()).isEqualTo(3);
  }
  ...
  void givenValidUser_whenLoggingUser_thenLoginAndReturnToken() {
    UsernamePasswordDto usernamePasswordDto = new UsernamePasswordDto(SAMPLE_USER_DTO.getUsername(),
        SAMPLE_USER_DTO.getPassword());
    ResponseEntity<HashMap> response = restTemplate.postForEntity(usersV1BaseUrl + "/login",
        usernamePasswordDto, HashMap.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    HashMap<String, String> body = response.getBody();
    assertThat(body).isNotNull();
    assertThat(StringUtils.countOccurrencesOf(body.get("token"), ".")).isEqualTo(2);
  ...
  void givenUsername_whenFindUser_thenPasswordMustBeEncrypted() {
    User user = userRepository.findByUsernameOrEmail(SAMPLE_USER_DTO.getUsername(),
        SAMPLE_USER_DTO.getEmail()).orElseThrow(EntityNotFoundException::new);
  ...
  void givenJwtTokenAndInValidUser_whenUpdateUser_thenReturnBadRequest() {
    ...
    ResponseEntity<ProblemDetail> exchange = restTemplate.exchange(URI.create(usersV1BaseUrl),
        HttpMethod.PATCH, new HttpEntity<>(userRequestDto, authHeader(jwtToken)),
        ProblemDetail.class);
  ...
  void givenJwtTokenAndValidUser_whenUpdateUser_thenReturnUpdatedUser() {
    UserRequestDto userRequestDto = UserRequestDto.builder().username(SAMPLE_USER_DTO.getUsername())
        .email(updatedEmail).password("longer121") // shorter password
        .build();
  ...
  void givenInvalidUsername_whenFindUser_thenReturnNotFound() {
    ResponseEntity<ProblemDetail> response = restTemplate.exchange(usersV1BaseUrl + "/dummy",
        HttpMethod.GET, createHttpEntity(null, jwtToken), ProblemDetail.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
  }
  ...
  void givenValidUsername_whenFindUser_thenReturnUser() {
    ResponseEntity<UserResponseDto> response = restTemplate.exchange(
        usersV1BaseUrl + "/" + SAMPLE_USER_DTO.getUsername(), HttpMethod.GET,
        createHttpEntity(null, jwtToken), UserResponseDto.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    UserResponseDto body = response.getBody();
    assertThat(body).isNotNull();
